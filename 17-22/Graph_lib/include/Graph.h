...
struct Function : Shape {
	...
	Function( std::function<double(double)>, double r1, double r2, Point xy, int count, double xscale, double yscale);
...
struct Lines : Shape {	// indepentdent lines
	...
	Lines(std::initializer_list<Point> lst) 
		: Shape{lst} { if (lst.size() % 2) error("odd number of points for Lines"); }
	void draw_lines() const;
